...
def asset_rel(slug: str, kind: str) -> str:
    """Path to a generated reference, relative to sequences/."""
    for group in (ROOT / "outputs" / "reference_assets").iterdir():
        candidate = group / f"{slug}_{kind}.png"
        if candidate.is_file():
            return "../" + candidate.relative_to(ROOT).as_posix()
    raise SystemExit(f"reference not found: {slug}_{kind}.png")
...
<Subject 1> is {c['cook']}, working at the counter of the kitchen in <Picture 1>.
...
[Shot 3] At 00:03.400, the shot cuts to a locked-off close-up at cooking height. The vessel of <Picture 3> fills the lower two thirds of frame over its heat source, already hot. The hands of <Subject 1> enter from the top of frame carrying the ingredient, tip it in once, and withdraw straight up out of frame empty. {c['first_heat']} The camera does not move at any point in this shot.
...
detailed_description:
[Shot 1] Locked-off close-up at cooking height, 50mm feel, continuing directly from <Picture 1>. The vessel of <Picture 4> fills the lower two thirds of frame over its heat source, which stays visible at the bottom of frame throughout. What went in during clip 01 is already colouring and moving with the heat. The hands of <Subject 1> enter from the top of frame carrying the remaining prepared components from <Picture 3>, exactly the size and cut that image defines, tip them in once among what is already cooking, and withdraw straight up out of frame empty. The added components settle against the hot surface, and the sound and movement of the cook change audibly as they land. The camera does not move at any point in this shot, and the framing does not drift.
...
<Subject 1>: fully_preserved. Same hands; face may enter softly at the end.
<Picture 2>: attribute_transfer. Light direction and material palette carry over; framing does not.
...
[Shot 3] At 00:03.400, the shot cuts to a locked-off medium close-up of the table. The finished dish of <Picture 4> sits in the foreground, the small printed card of <Picture 6> standing far back near the edge, legible but never dominant, <Subject 1> present behind in soft focus. {c['final_lift']} The camera does not move and the focus does not rack. The shot ends with the piece held above the finished dish.
...
def build_sequence(cfg: dict) -> dict:
    slug = cfg["slug"]
    a = {k: asset_rel(slug, k) for k in KINDS}
