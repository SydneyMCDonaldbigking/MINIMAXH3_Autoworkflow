Create a clean high-quality vertical reference image for a premium cooking commercial, 1080x1920.

PURPOSE: an extra process-state reference for MiniMax H3 Tom Yum Goong, Thai hot and sour prawn soup, covering a moment the four standard references do not: the raw prawns going into the finished broth, which happens at the end of clip 02.

SCENE: a wide steel pot over a gas flame in the kitchen described by the character reference.

SUBJECT: the wide steel pot of amber-red tom yum broth at a fast rolling boil on its flame, seen from just above the rim, with whole raw prawns dropping into it. The prawns still in the air are grey and translucent; the ones that have landed are already turning orange at the shell edges. Lemongrass, galangal and straw mushrooms turn in the broth around them. This is a cooking pot on a burner, not a serving bowl, and there is no table, no garnish and no lime wedge in frame.

COMPOSITION: vertical 9:16, close-up at vessel height looking slightly down. The action and the heat source are both legible.

STYLE: photorealistic commercial food cinematography, high quality, realistic hands, natural textures, rich but clean colour grade.

NEGATIVE: subtitles, title cards, watermarks, floating logo, any printed card, any brand text, invented lettering, fake app UI, distorted face, extra fingers, warped cookware, messy kitchen, no lettering, logos or model names on burners, stoves, pots, pans, appliances, packaging or containers, no readable text anywhere in frame, hands in frame, a finished plated dish, a serving bowl, coconut milk, a white or creamy broth, a thick curry, prawns cooked with the aromatics from the start, overcooked grey prawns.
