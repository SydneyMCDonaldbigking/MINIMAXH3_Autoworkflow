...
from setup_links import print_h3_runtime_setup
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prepare-only", action="store_true", help="Show an existing prepared brief or rehearsal. Real media understanding is done by the host agent.")
    ...
    parser.add_argument("--h3-dry-run", action="store_true", help="Build the MiniMax H3 sequence package and runner command without submitting to ComfyUI")
    parser.add_argument(
        "--confirmed-brief",
        action="store_true",
...
def localize_rehearsal_payload_en(payload: dict) -> dict:
    ...
    payload["rewrite_plan"] = {
        "rewrite_strategy": {
            ...
            "risk_controls": [
                ...
                "keep the result as three stable five-second H3 clips",
            ],
        },
        ...
        "prompt_package": {
            "base_reference_summary": "Create three five-second 9:16 MiniMax H3 clips for a watermelon drink ad from the product image, borrowing the template's ingredient hook, liquid texture proof, and lifestyle satisfaction close.",
            "style_constraints": {
            ...
            "structure_constraints": {
                ...
                "total_duration": "three independent five-second clips",
        ...
        "generation_prompt": {
            "prompt": "Rewrite the product image into three five-second 9:16 MiniMax H3 Ref2VA clips for a watermelon drink ad. Borrow only the template's shot rhythm, ingredient hook, liquid texture proof, lifestyle drinking satisfaction, and visual CTA. Keep the source image as product truth: straight clear glass, red watermelon drink, ice cubes, fruit pulp, condensation, mint, and watermelon pieces. Do not inherit the template's grape drink, bottle, brand, flavor, claims, subtitles, or shopping UI. No subtitles, no captions, no on-screen text, no stickers, no labels, no price tags. Export silent clips by default; use soundscape only to guide motion.",
    ...
    payload["rewrite_brief"] = {
        ...
        "template_extend_to_15s_plan": "Keep the template's first-five-second logic in clip 01, then use clips 02-03 for product details, use context, and a product-in-hand visual CTA.",
    ...
    payload["prompt_preview"] = {
        "user_summary": "\n".join(
            [
                ...
                "- Output: 9:16, 1080x1920 native H3 package",
                "- Export audio: no",
...
def print_request_summary(request: RewriteRequest, *, language: str = "zh") -> None:
    if language == "en":
        ...
        print(f"- H3 server: {request.h3_server}")
        print(f"- Export audio: {'on' if request.generate_audio else 'off'}")
        return
    print("请求已校验。")
    ...
    print(f"- H3 server：{request.h3_server}")
    print(f"- 导出音频：{'开启' if request.generate_audio else '关闭'}")
...
def write_rehearsal_result(path: Path, prepared: PreparedRewrite, *, language: str = "zh") -> None:
    ...
    if language == "en":
        payload.update(
            {
                "summary": "Example rehearsal result: this is a bundled watermelon-drink sample video for the no-cost confirmation flow. This rehearsal does not call MiniMax H3 or ComfyUI.",
                "manual_review_checklist": [
                    ...
                    "Confirm clip 01 has a clear hook and clips 02-03 extend texture, use context, and visual close.",
                    "Confirm the H3 package would be reviewed as independent clips before final manual editing.",
                ],
            }
        )
    ...
    payload["provider"] = "minimax_h3_rehearsal"
    payload["h3_clip_paths"] = [str(REHEARSAL_PREVIEW_PATH)]
...
def print_rehearsal_result(path: Path, *, language: str = "zh", media_style: str = "codex") -> None:
    ...
    if language == "en":
        ...
        print("This is the bundled watermelon-drink sample video. This rehearsal does not call MiniMax H3, ComfyUI, or the GPU server.")
        print(media_markdown("Example result video", REHEARSAL_PREVIEW_PATH, style=media_style))
        print("Provider: minimax_h3_rehearsal")
        english_checklist = [
            "Watch the bundled real watermelon-drink sample video first, then review the task information.",
            "Confirm the product identity comes from the watermelon drink product image, not the grape-drink template.",
            "Confirm there are no subtitles, price tags, shopping buttons, stickers, or template brand remnants.",
        ...
        return
    print("\n示例彩排结果（不是真实生成）")
    print("这里展示的是 skill 内置西瓜果饮样例视频；本次彩排不调用 MiniMax H3、ComfyUI 或 GPU server。")
...
def print_h3_advantages_and_setup(*, language: str = "zh", media_style: str = "codex") -> None:
    if language == "en":
        print("\nWhy MiniMax H3 is the production target")
        print(
            "The local workflow uses MiniMax H3 Ref2VA through ComfyUI so product, scene, character, and state references stay under our control. "
            "The production default is three independent 5s vertical clips at 1088x1920, Turbo LoRA, 8 steps, no exported audio, then optional crop to 1080x1920. "
            "This matches the current A100-40G workflow and leaves final editing to the user."
        )
        print_h3_runtime_setup(language)
        return
    print("\n为什么最终生产用 MiniMax H3")
    print(
        "当前本地工作流通过 ComfyUI 跑 MiniMax H3 Ref2VA，可以把商品、场景、人物、状态参考都控制在我们自己的素材栈里。"
        "生产默认是三段独立 5 秒竖版片段：1088x1920、Turbo LoRA、8 steps、不导出音频，再按需要裁到 1080x1920。"
        "这和现在 A100-40G 的稳定路线一致，也方便你后续手动剪辑。"
    )
    print_h3_runtime_setup(language)
...
def print_confirmed_missing_key_snapshot(prepared: PreparedRewrite, *, language: str = "zh") -> None:
    ...
    if language == "en":
        ...
        print("- This confirmed prepared brief can be sent to MiniMax H3 after the ComfyUI server and local references are ready; media analysis will not be repeated.")
        return
    ...
    print("- ComfyUI server 和本地参考图准备好后，会直接用这份 confirmed prepared brief 提交 MiniMax H3，不重新分析素材。")
...
def print_agent_preparation_required(request: RewriteRequest, *, language: str = "zh") -> None:
    if language == "en":
        ...
        print("Video understanding and rewrite planning are performed by the host agent, not by the H3 runner.")
        ...
        print("The confirmed prepared brief is then compiled into three MiniMax H3 Ref2VA clip prompts and a sequence JSON.")
        return
    print("\n需要先由 Agent 完成分析。")
    print("视频理解和改写规划由当前 Agent 完成，不交给 H3 runner。")
    print("请先由当前 Agent 理解模板视频和商品图，写出 prepared JSON，再用 --prepared-input-json 交给 runner。")
    print("确认后的 prepared brief 会被编译成三段 MiniMax H3 Ref2VA prompt 和 sequence JSON。")
...
def main() -> None:
    ...
    if args.rehearsal:
        ...
        if show_prepared_stage:
            if language == "en":
                print("No-cost full-flow rehearsal: replaying bundled prepared/result artifacts without calling MiniMax H3 or ComfyUI.")
            else:
                print("无成本完整彩排：使用 skill 内置示例 prepared/result 回放，不调用 MiniMax H3 或 ComfyUI。")
    ...
    if args.setup_only:
        ...
        print_h3_advantages_and_setup(language=language, media_style=args.media_style)
        return
    ...
    if prepared is None:
        ...
        print("Loaded prepared JSON from agent analysis; no provider media analysis will be called." if language == "en" else "已读取由 Agent 分析得到的 prepared JSON；不会调用外部 provider 做素材理解。")
        if patch_path:
    ...
    if response.h3_clip_paths:
        for index, clip_path in enumerate(response.h3_clip_paths, start=1):
            label = f"Generated clip {index:02d}" if language == "en" else f"生成片段 {index:02d}"
            print(media_markdown(label, clip_path, style=args.media_style))
    elif response.rewritten_video_local_path:
    ...
    if response.h3_run_id:
        print(f"H3 run ID: {response.h3_run_id}" if language == "en" else f"H3 run ID：{response.h3_run_id}")
    if response.h3_manifest_path:
        print(f"H3 manifest: {response.h3_manifest_path}" if language == "en" else f"H3 manifest：{response.h3_manifest_path}")
