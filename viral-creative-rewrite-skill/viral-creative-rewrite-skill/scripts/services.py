...
MUSIC_PROMPT = (
    "MiniMax H3 生产默认导出静音片段；仍然在 prompt 的 soundscape/music 段描述节奏和动作冲击，"
    "但保存时使用 no_audio，不生成可识别音乐、歌词、人声或口播。"
)
MUSIC_PROMPT_EN = (
    "MiniMax H3 production exports silent clips by default; still describe soundscape/music for motion and pacing, "
    "but save with no_audio and do not generate identifiable music, lyrics, vocals, or voiceover."
)
...
def voice_language_blocking_message(prepared: PreparedRewrite, *, language: str = "zh") -> str:
    ...
    if language == "en":
        return (
            "Generation is blocked before MiniMax H3 because this is a human_voiceover template but the template spoken language is not confirmed. "
    ...
    return (
        "已在提交 MiniMax H3 前阻断：这是口播模板，但模板口播语言还没有被明确确认。"
...
def person_context_blocking_message(prepared: PreparedRewrite, *, language: str = "zh") -> str:
    ...
    if language == "en":
        return (
            "Generation is blocked before MiniMax H3 because this human-presenter template does not have a reliable template person / market visual context. "
    ...
    return (
        "已在提交 MiniMax H3 前阻断：这是真人出镜/口播模板，但模板人物外观/市场语境没有可靠确认。"
...
def profile_audio_unresolved_blocking_message(prepared: PreparedRewrite, *, language: str = "zh") -> str:
    """Block before MiniMax H3 when a human-presenter template's audio track was never confirmed.
    ...
    if language == "en":
        return (
            "Generation is blocked before MiniMax H3 because this is a human-presenter template "
    ...
    return (
        "已在提交 MiniMax H3 前阻断：这是真人出镜模板，但音轨是否含口播（human_demo vs human_voiceover）尚未确认。"
...
def generation_contract_blocking_message(prepared: PreparedRewrite, *, language: str = "zh") -> str:
    """Final gate before MiniMax H3: block prompt/package contradictions, not just missing keys."""
    ...
    if brief.template_profile == "visual_product_texture":
        ...
        if hit:
            if language == "en":
                return (
                    "Generation is blocked before MiniMax H3 because a visual_product_texture template contains voiceover/lip-sync hard constraints. "
                    f"Remove the conflicting prompt fragment first: {hit}"
                )
            return f"已在提交 MiniMax H3 前阻断：视觉质感型模板不应包含口播/口型同步硬约束。请先移除冲突片段：{hit}"
    ...
    if voice_kind == "en":
        ...
        if hit:
            if language == "en":
                return (
                    "Generation is blocked before MiniMax H3 because the template voiceover language is English but the prompt still contains Chinese-voiceover instructions. "
                    f"Conflicting fragment: {hit}"
                )
            return f"已在提交 MiniMax H3 前阻断：模板口播语言是 English，但 prompt 仍包含中文口播指令：{hit}"
    if voice_kind == "zh":
        ...
        if hit:
            if language == "en":
                return (
                    "Generation is blocked before MiniMax H3 because the template voiceover language is Chinese Mandarin but the prompt still contains English-voiceover instructions. "
                    f"Conflicting fragment: {hit}"
                )
            return f"已在提交 MiniMax H3 前阻断：模板口播语言是中文普通话，但 prompt 仍包含英文口播指令：{hit}"
    return ""
...
def _media_block(value: str, media_type: str) -> dict:
    raise RuntimeError(
        "Provider media analysis has been removed. "
...
def _print_analysis_readout(title: str, analysis: VideoAnalysis, *, language: str = "zh") -> None:
    ...
    if language == "en":
        ...
        if analysis.extend_to_15s_plan:
            print(f"- H3 three-clip extension: {analysis.extend_to_15s_plan}")
    ...
    if analysis.extend_to_15s_plan:
        print(f"- H3 三段延展：{_zh_display_text(analysis.extend_to_15s_plan)}")
    if analysis.replication_script:
...
def _print_brief_readout(brief: RewriteBrief, *, language: str = "zh") -> None:
    if language == "en":
        ...
        print(f"- H3 three-clip plan: {brief.template_extend_to_15s_plan or 'unknown'}")
    ...
    print(f"- H3 三段方案：{_zh_display_text(brief.template_extend_to_15s_plan) or '未识别'}")
    if brief.template_profile:
...
def _build_prompt_preview_user(request: RewriteRequest, plan: RewritePlan, brief: RewriteBrief | None = None) -> str:
    if _is_en(request):
        lines = [
            ...
            f"- Export audio: {'yes' if request.generate_audio else 'no'}",
    ...
    lines = [
        ...
        f"- 是否导出音频：{'是' if request.generate_audio else '否'}",
...
def _print_decision_gate_footer(*, language: str = "zh", include_detail_option: bool = True) -> None:
    ...
    """
    if language == "en":
        print("If you want to continue, reply: confirm generation. This will submit three MiniMax H3 clips to the configured ComfyUI server and may take around 40 minutes on an A100-40G.")
        ...
        return
    print("如果你要继续，请回复：确认生成。确认后会向已配置的 ComfyUI server 提交三段 MiniMax H3 片段；A100-40G 上通常约 40 分钟。")
    if include_detail_option:
...
def _print_compact_decision_brief(prepared: PreparedRewrite, *, language: str = "zh") -> None:
    ...
    if language == "en":
        print("Real analysis preview is complete. MiniMax H3 has not been called yet, and no new video has been generated.")
        print("Brief summary:")
        print(
        ...
        print(f"- Default output: {request.output_ratio}, {request.output_resolution}, three independent 5s clips, {'audio on' if request.generate_audio else 'audio off'}, H3 Turbo 8 steps, crop-ready 1088x1920 to 1080x1920.")
    ...
    print("真实分析预览已完成，当前还没有调用 MiniMax H3，也还没有生成新视频。")
    print("本次 brief 摘要：")
    print(
        "- 模板结构：借模板的"
    ...
    print(f"- 输出默认：{request.output_ratio}，{request.output_resolution}，三段独立 5 秒片段，{'带' if request.generate_audio else '不带'}音频，H3 Turbo 8 steps，1088x1920 生成后可裁到 1080x1920。")
...
def analyze_video(video_path: str, *, video_label: str, media_type: str = "video", ui_language: str = "zh") -> VideoAnalysis:
    raise RuntimeError(
        "Provider video understanding has been removed. "
...
def build_rewrite_plan(request: RewriteRequest, *, viral_analysis: VideoAnalysis, source_analysis: VideoAnalysis) -> RewritePlan:
    raise RuntimeError(
        "Provider rewrite planning has been removed. "
...
def _finalize_generation_prompt(request: RewriteRequest, plan: RewritePlan, brief: RewriteBrief | None = None) -> GenerationPrompt:
    ...
    prompt = (
        ...
        "Prepare this as a MiniMax H3 Ref2VA production plan: three independent five-second vertical clips, "
        "each with exactly three planned shots, a strong opening hook, clear product proof, visible satisfaction moments, "
        "and a product-visible final handoff for manual editing.\n"
        "Treat all storyboard entries as approximate creative beats, not frame-accurate editing instructions. "
        "Do not attempt to reproduce every micro-shot literally. Consolidate small adjacent actions into stable H3 physical actions so the product remains recognizable. "
        "Avoid excessive scene changes, prop changes, stacked camera moves, or complex hand/tool motion that could destabilize the source product identity.\n"
        ...
        "Never end with blank space, white screen, black screen, a plain-color fade, or a product-free empty shot. The final second of clip 03 must keep the source product visibly present in a complete visual CTA frame.\n"
        "Use MiniMax H3 production defaults unless the user overrides them: R2V/Ref2VA, 1088x1920 native vertical, 5.0 seconds per clip, 8 steps, Turbo LoRA on, turbo_low_vram on, no_audio on, ref_image_size match, and crop-ready delivery to 1080x1920. "
        "Clip 01 covers the first-five-second hook and product identity; clip 02 covers product proof, texture, transformation, or use mechanics; clip 03 covers satisfaction close and visual CTA. "
        "If the template is shorter than 5 seconds, preserve its action logic and satisfaction mechanism in clip 01, then add source-product proof shots without changing product identity.\n"
...
def generate_rewritten_video_artifact(request: RewriteRequest, plan: RewritePlan) -> GeneratedVideoArtifact:
    from h3_runtime import run_h3_sequence

    _log("MiniMax H3 generation start")
    h3_result = run_h3_sequence(request, plan)
    task_id = str(h3_result.get("run_id") or "") or None
    cache_key = f"h3-{h3_result.get('sequence_id', 'sequence')}-{h3_result.get('run_id', 'run')}"
    local_path = str(h3_result.get("local_video_path") or "")
    if _debug_artifacts_enabled() and local_path and Path(local_path).suffix.lower() in {".mp4", ".mov", ".webm", ".mkv", ".avi"}:
    ...
    artifact = GeneratedVideoArtifact(
        ...
        clip_paths=list(h3_result.get("clip_paths") or []),
        manifest_path=str(h3_result.get("manifest_path") or ""),
        sequence_config_path=str(h3_result.get("sequence_path") or ""),
        prompt_files=list(h3_result.get("prompt_files") or []),
        runner_log_path=str(h3_result.get("runner_log_path") or ""),
        server=str(h3_result.get("server") or ""),
    )
    _log(f"MiniMax H3 generation done run_id={task_id or 'unknown'} local_path={local_path}")
...
def execute_rewrite(prepared: PreparedRewrite) -> RewriteVideoResponse:
    ...
    result = RewriteVideoResponse(
        ...
        h3_run_id=artifact.task_id,
        h3_manifest_path=artifact.manifest_path,
        h3_sequence_json_path=artifact.sequence_config_path,
        h3_clip_paths=artifact.clip_paths,
